...
| D-1 | Confirm `api.registerChannelImpl` shape supports discord's 11-function bridge. Already used by WA, should be a no-op assert. | 0–1 | trivial |
| D-2-lite | Move runtime-bridge population from `src/plugins/runtime/index.ts:258-265` into `extensions/discord/index.ts` via `api.registerChannelImpl(...)`. Add `whenRegistered()` gating to the 5 hottest consumers. | ~7 | low — bridge stays, just shifts ownership |
