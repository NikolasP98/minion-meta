...
| D-2-lite ✅ 2026-05-20 | Shifted ownership of `runtime.channel.discord` from core to the discord plugin. `src/plugins/runtime/index.ts` no longer hard-imports `impl/discord/*` or `channels/plugins/actions/discord` — the slot is now `discord: {} as unknown as PluginRuntime["channel"]["discord"]` (WA's empty-shape pattern). `src/plugin-sdk/index.ts` gained the "Discord channel runtime surface" block re-exporting the 10 bridge functions. `extensions/discord/index.ts` now calls `api.registerChannelImpl("discord", { ... })` before `registerChannel(...)`. No `whenRegistered()` gating needed — runtime construction never reads `runtime.channel.discord.*` synchronously; consumers read at message-handling time, after plugin bootstrap. Verified: typecheck clean for discord, register-channel-impl tests pass, gateway build succeeds. Commit `b825ed2af` on DEV. | 3 | low — bridge stays, just shifts ownership |
| D-3 ✅ 2026-05-20 | Built `extensions/discord/ui/` mirroring WA's scaffolding. Two iframe entries (`index.html` for `settings.plugins` slot, `control.html` for `plugins.controlCenter`). Server: `extensions/discord/src/dashboard-rpcs.ts` with `accountsSummary` / `recentMessages` / `volumeBuckets` registered via `api.registerGatewayMethod(...)` in `extensions/discord/index.ts`. Account summary uses discord-specific fields: `tokenSource` (env\|config\|none) instead of WA's QR/authAge — "linked" simply means `tokenSource !== "none"`. Settings panel is a minimal read-only accounts table; a full guilds/channels/reactions/actions editor is deferred. Verified: vite build clean (3 chunks, 1.13MB control bundle), gateway build clean, typecheck shows only the same pre-existing `.svelte` module-typing warnings WA UI has. Commit `b38a82fda` on DEV. | new dir, ~14 files | additive |
...
| T-2-lite ✅ 2026-05-20 | Shifted ownership of `runtime.channel.telegram` from core to the telegram plugin. `src/plugins/runtime/index.ts` lost 6 value imports of `impl/telegram/*` + `channels/plugins/actions/telegram`; slot is now `telegram: {} as unknown as PluginRuntime["channel"]["telegram"]`. `src/plugin-sdk/index.ts` gained the "Telegram channel runtime surface" block re-exporting the 8 bridge fns. `extensions/telegram/index.ts` calls `api.registerChannelImpl("telegram", { ... })` before `registerChannel(...)`. Boot-order risk was a non-issue: CLI doctor + security audit consume `channels/telegram-helpers/*` (now moved per T-0) and `channels/impl/telegram/accounts.ts` directly — neither touches the `runtime.channel.telegram.*` bridge surface. Surfaced one T-0 straggler during typecheck: `src/channels/plugins/onboarding/telegram.ts:12` import path updated `telegram/api.js → telegram-helpers/api.js`. Verified: typecheck clean, register-channel-impl tests pass, gateway build succeeds. Commit `2e86b72d3` on DEV. | 4 | low — bridge stays, just shifts ownership |
| T-3 ✅ 2026-05-20 | Built `extensions/telegram/ui/` mirroring `extensions/discord/ui/` wholesale (discord itself mirrored WA). Two iframe entries (`index.html` / `control.html`). Server: `extensions/telegram/src/dashboard-rpcs.ts` with `accountsSummary` (tokenSource widened to `"env" \| "tokenFile" \| "config" \| "none"` since telegram supports `BOT_TOKEN_FILE`) / `recentMessages` / `volumeBuckets`, registered via `api.registerGatewayMethod(...)`. Plugin manifest gains `ui[]` block. Verified: vite build clean (1.13MB control bundle), gateway build clean. Commit `44e10aa3a` on DEV. | new dir, ~14 files | additive |
