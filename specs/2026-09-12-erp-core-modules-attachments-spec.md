...
| S12 | follow-ups: abandoned-upload sweeper tick, owner-scoped CRM standardize + insights, fixture seed type fix | `~/.cache/claude-tmp/erp-s12` | `feat/erp-s12-sweeper-owner-scope` → hub #268 | MERGED `25e2bb75` | ✅ prod `25e2bb75` (deployment success; sweep tick 401 unauth; login 200) |
...
- **B2 bucket CORS for browser-direct PUT** (S2/S3) — script ready: `cd minion_hub && bun --env-file=.env ~/.cache/claude-tmp/b2-cors/put-cors.ts` (the agent cannot load the credentials; owner runs it): the presigned-PUT path needs a CORS rule on the `minionhub` bucket allowing `PUT` (+ `Content-Type` header) from `https://hub.minion-ai.org` (and preview origins if wanted). Until then the client helper falls back to the server-proxied `POST /api/files` for files ≤ 4 MB; larger files fail with the PUT error. Not verifiable from this session (needs B2 credentials). See also [[backblaze-b2-bucket-audit]].
- ~~Sweeper for abandoned uploads~~ shipped in #268 (`GET /api/attachments/sweep/tick`). **Owner:** add the netcup crontab line `0 * * * * curl -s -m 50 -H "Authorization: Bearer $CRON_SECRET" https://hub.minion-ai.org/api/attachments/sweep/tick >/dev/null`, then flip the manifest entry's `wiring` from `unscheduled` to `netcup`.
- ~~Owner-scope gaps~~ closed in #268 (standardize scoped; insights cache key carries ownerId and org-wide rollups are gated for scoped roles). Remaining `TODO(handoff)` in `crm-insights-dashboard.service.ts`: word-frequency/sentiment/win-index refresh pipelines have no owner dimension.
