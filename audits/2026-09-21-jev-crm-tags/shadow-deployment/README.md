...
Operational note: live request/context/receipt bodies are scrubbed after 30 days. Rolling same-host backups expire 30 days after backup creation, so an older context can remain in those backups for up to roughly 60 days. Immediate erasure propagation across backups is an explicit follow-up, not claimed implemented. On deployed commit `0375db34`, dedicated shadow, general unit, SQL, build and QA-stack checks have passed; Vercel preview was still pending at the final check and is not this sidecar deployment.
