...
for(const file of [...readdirSync(here).filter(x=>x.endsWith('.md')).map(x=>join(here,x)),join(root,'specs/2026-09-21-hub-optimistic-interactions-motion-spec.md'),join(root,'specs/2026-09-21-hub-optimistic-interactions-motion-spec.review.md')]){
...
const inventory=JSON.parse(readFileSync(join(here,'source-inventory.json'),'utf8'));
const changed=[];
for(const [path,hash] of inventory.files.map(f=>[f.path,f.sha256])){
 const full=join(root,path);
 if(!existsSync(full)||createHash('sha256').update(readFileSync(full,'utf8')).digest('hex')!==hash)changed.push(path);
}
for(const f of JSON.parse(readFileSync(join(here,'catalog.json'),'utf8'))){
 const file=join(root,f.source);
 if(!existsSync(file)||f.line>readFileSync(file,'utf8').split('\n').length)errors.push({family:f.id,anchor:f.source});
}
const html=readFileSync(join(root,'.lavish/hub-optimistic-interactions-2026-09-21.html'),'utf8');
