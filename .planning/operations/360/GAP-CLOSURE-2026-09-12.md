...
The replacement worker's bare compiled build reproduced a shutdown defect: after SIGTERM its listener closed but the process remained alive beyond95seconds, exceeding the production90-second stop window.10-15 now includes Node-only quiesce/drain and graceful resource cleanup before adoption.09-09 separately repairs six real-handler diagnostic leakage cases and adds native principal revocation qualification; prior PGlite tests are not relabeled as native PostgreSQL/provider proof.

Read-only attachment migration preflight on the configuredPG17.6 target found zero existing attachment links, zero managed files, zero tenant/key collisions and no lifecycle table. This is aggregate target evidence, not Vercel environment identity proof; no migration was applied. The worker shutdown baseline eventually exited naturally at120.029seconds, consistent with its pool idle timeout and longer than the service90-second limit.

Vercel current production project configuration was independently matched to the same configured Supabase project and pooler/database as the Node worker and catalog preflight. Only sanitized identity was retained; the temporary environment file was removed. This closes target-configuration uncertainty for the next rollout; it is not inspection of an already-running Vercel process. Receipt: gap-closure/evidence/vercel-production-target.json.
