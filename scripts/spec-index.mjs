...
import { isIsoDate, readRouting, validateSliceTags } from './routing.mjs';

// routing.yml is slice_tags' single source of truth (node scripts/routing.mjs validate).
// The general `tags` vocabulary itself stays topics.json's (above) — slice_tags is validated
// separately below and does not affect topic resolution.
...
export const ARRAY_FIELDS = ['repos', 'tags', 'related', 'slice_tags'];
...
function main() {
	...
	for (const name of readdirSync('specs')
		...
		// slice_tags cutoff below — reject it here, unconditionally, rather than let it fail open.
		...
		// The slice is the routable unit (§4b): a one-slice dev run selects its checks and
		// reviewers from the slice's tags, so an unknown or absent slice tag is a misrouted run,
		// not a nit. slice_tags values are the routing.yml work-type enum, not topics.json's —
		// validateSliceTags checks that enum and that `tags` is exactly their union.
		for (const error of validateSliceTags(routing, fm, body)) errors.push(`${name}: ${error}`);

		if (check) {
			// "created" is already validated unconditionally above (the slice_tags cutoff needs it
			// fail-closed even outside --check); only "updated" is check-only.
			if (fm.updated && !isValidISODate(fm.updated))
				errors.push(`${name}: "updated" is not a valid ISO calendar date (YYYY-MM-DD): "${fm.updated}"`);
