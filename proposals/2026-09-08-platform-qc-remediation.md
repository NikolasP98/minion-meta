...
Private UI commits f5941c14 and e31ca501 must be integrated together. The first alone freezes pixel spawn/removal lifecycle under reduced motion and is rejected. The correction passes actual OfficeState lifecycle and three-engine browser cases. Agent selection/task assignment now have keyboard controls and a native dialog; these component fixtures do not authenticate or mount the full Workshop.

The next bounded slice owns motion-preference.ts, agent-sprite.ts, habbo-renderer.ts, habbo-element-sprite.ts and real Pixi/Rapier fixture tests. Preserve physics, task/activity scheduling, user-driven camera, base sprite geometry/alpha and listener/ticker cleanup. Relationship/element keyboard operations and concrete chart-consumer drilldowns remain open. Full login/session/org-isolation/Home/Calendar/CRM journeys require an identified non-production Supabase provider, canonical schema and disposable users; the user was asked for that target. Synthetic fixture data cannot close 13-03. Keep 13-04 task2 partial until all engine/consumer evidence and bounded completion plans exist. Native assistive-technology and deployed behavior need independent acceptance.

### 2026-09-12 bridge runtime qualification

Meta PR400 merged into dev as6103e982. Hosted run34714453995 passed216 tests with zero skips in the SHA-pinned official Node22.13.0 image, actual SQLite3.47.2, and no runtime flags. Emitted journal reopen/ACK passed. All qualification/source files match the tested tree; only unrelated rankings/index.json differs between the tested PR tree and final dev merge tree. Actual workstation-image adoption and npm generation remain open. Published shared0.12.0/bridge0.2.0 are earlier code, so this gate does not certify their runtime behavior.


## 2026-09-12 governance and ACP qualification limits

Factory PR187 adds actual MCP/tool-host/runner/native-SQLite effect-boundary tests. Unauthorized token/tool/runner capability requests cause no admission; same-key replay after killing and reopening the owned disposable child causes no duplicate phase effect; changed payload and revoked capability are denied. It qualifies phase admission, not external executor effects. The11-05 corpus still needs budget, mutable tool/schema/model/image identity, actual expiry policy and external execution coverage. Its new runner-effects test TODO points here. Promotion and live flags are recorded separately; no activation is inferred from tests.

11-04 ACP SDK continuation: officialSDK1.4.0 and exactZod4.3.6 peer are dev-only qualification dependencies. Candidate code lives outside production src/exports/dist. Stable protocol dispatch and inbound schemas use the SDK; consumed result fields retain explicit validation because generic SDK requests do not validate all response schemas. Production adoption requires persistent caller-to-ACP session mapping, load/restart recovery and late-update fences. Caller sessionId is not established as correlation-only, so fresh-session-per-turn cannot be adopted silently. Existing bridge runtime remains unchanged. Credential-free official example-agent evidence does not close actual provider/harness conformance. Candidate exact-site TODO points here.

UI13-04 Task2 now has native pixel/Pixi/Rapier evidence and bounded consumer plans. Remaining relationships/elements/camera keyboard equivalents and concrete chart filter/legend/Sankey tasks are specified in.planning/phases/13-ui-qualification/13-REMAINING-CONSUMER-PLANS.md. Component fixtures do not establish full authenticated Workshop or native assistive-technology acceptance.


## 11-04 ACP SDK continuation

The test-only adapter in `packages/shells-bridge/test/qualification/sdk-acp-client.ts` qualifies official SDK 1.4.0 over credential-free child-process stdio. It is excluded from production builds and package files; SDK and Zod remain exact development pins. Existing runtime `src/acp-client.ts` and `src/bridge.ts` are unchanged. This does not complete 11-04 or qualify deployed providers.

Caller `sessionId` is required but its conversation semantics are not defined in `packages/shared/src/gateway/shells.ts:446–458`. The payload contains one text or multimodal input, not full history. No Shells invocation consumer was found in the reviewed committed Hub candidate. The superseded Golden Agents spec describes long-lived harness-owned state. A fresh ACP session on every invoke must not be silently adopted.

Bounded follow-up ownership: shared Shells session capability/continuation contract; bridge configuration and persistent caller-to-provider mapping adjacent to the existing journal (separate schema and transaction design); ACP initialize/new/load/resume capability handling; bridge invoke/cancel/update call sites; sender journal admission/replay/uncertainty tests; selected caller integration. Preserve gateway-issued shell/run/session/invocation identity and current run ownership. Never replay an unresolved prompt or invent provider conversation recovery after process restart.

Required decisions before implementation: explicit caller request for new versus continuation; isolation key including authenticated shell/organization and caller session; negotiated stable load/resume support and explicit refusal when unavailable; persistence/backup ownership and bounded retained mappings; prohibition of sharing provider sessions across caller identities; correlation policy for late same-session frames. ACP session/update carries sessionId but no runId, so reusing provider sessions requires a justified transport/session fence. A new run must not inherit old updates merely because the caller session string matches.

Acceptance gates: two prompts preserve demonstrable prior context; separate caller sessions cannot read each other's history; restart/load preserves context or refuses explicitly; late prior-turn updates cannot mutate a new run; duplicate invocation replays journal evidence without another prompt; cancel notification alone never counts as acknowledgement; protocol closure while the child remains alive stays uncertain; real child exit is observed from close. Run these through the actual local tool host and selected SDK peer before external provider acceptance. Credentialed harness, VM image, gateway adoption and production deployment remain distinct gates.

Upstream SDK example negative: cancellation during model work returns cancelled, but cancellation during a pending permission returns end_turn. Keep this recorded result; do not report it as cancellation acknowledgement or a passing denial/no-side-effect governance gate.

## 14-07 follow-up (2026-09-15): generated-bridge rebuild cannot run on hub CI
