...
â see `specs/2026-08-12-minion-base-v2-sdlc-kanban-spec.md` Â§1 "Kanban derivation"), but
...
worse than no view â you make decisions off a snapshot without knowing its age. Two small
...
- **refresh timer** â 5 min, triggers the data refetch (SvelteKit `invalidate`/`invalidateAll`
  on the kanban load), only running while `document.visibilityState === 'visible'`.
- **display timer** â short tick (~1sâ15s, implementer's call) that only re-renders the
...
styled with existing semantic tokens from `src/lib/design/tokens.css` â no raw values.
...
decide how to handle this â cache-bust the refetch, shorten `s-maxage`, or accept it â and
...
- Timers are cleared on component destroy â no leaks, no fetches after leaving the board.
